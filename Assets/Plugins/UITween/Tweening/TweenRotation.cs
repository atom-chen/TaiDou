...
public class TweenRotation : UITweener
{
	...
	Transform mTrans;
    public System.Action<GameObject, Quaternion> mValueUpdate;
    ...
    protected override void OnUpdate (float factor, bool isFinished)
	{
		value = quaternionLerp ? Quaternion.Slerp(Quaternion.Euler(from), Quaternion.Euler(to), factor) :
            ...
            from.x * (1f - factor) + to.x * factor,
            from.y * (1f - factor) + to.y * factor,
			from.z * (1f - factor) + to.z * factor));

        if(mValueUpdate != null)
        {
            mValueUpdate.Invoke(this.gameObject,value);
        }
	}
