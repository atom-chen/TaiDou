...
public abstract class UITweener : MonoBehaviour
{
    ...
    /// <summary>
    /// Awake set from value
    /// </summary>
    [HideInInspector]
    public bool awakeFrom = true;
    /// <summary>
    /// set active
    /// </summary>
    [HideInInspector]
    ...
    public delegate void OnTweenerEnd(GameObject go);
    public OnTweenerEnd Call
    {
        get { return call; }
        set { call = value; }
    }

    OnTweenerEnd call = null;
	...
	void Update ()
	{
		...
		else if (style == Style.PingPong)
		{
			...
			else if (mFactor < 0f)
			{
                ...
                //onFinished在此处有消耗
                if (Call != null) Call(this.gameObject);
			}
		}
        else if (style == Style.PingPongOnce)
        {
            if (mPingPongOnceStep == 3)
            { //end
	...
	public void Play (bool forward)
	{
        ...
        if (style == Style.PingPongOnce) mPingPongOnceStep = 0;
		enabled = true;
		Update();
	}
